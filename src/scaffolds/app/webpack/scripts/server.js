...
function setupCompiler(webpckConfig) {
  // "Compiler" is a low-level interface to Webpack.
  // It lets us listen to some events and provide our own custom messages.
  compiler = webpack(webpckConfig);
  ...
  compiler.hooks.done.tap('server', function(stats) {
    ...
    if (showInstructions) {
      ...
      console.log();
      console.log('  ' + chalk.cyan(protocol + '://' + host + ':' + port + '/' + pageDir[0] + '.html'));
      console.log();
      console.log();
...
function runDevServer(webpckConfig) {
  ...
  devServer.listen(port, host, (err, result) => {
    ...
    openBrowser(protocol + '://' + host + ':' + port + '/' + pageDir[0] + '.html');
  });

}

function run(webpckConfig) {
  setupCompiler(webpckConfig);
  runDevServer(webpckConfig);
}
